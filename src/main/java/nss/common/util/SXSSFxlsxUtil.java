...
public class SXSSFxlsxUtil{

	final static Logger logger = LoggerFactory.getLogger(SXSSFxlsxUtil.class);
	...
	private SXSSFSheet newSheet = null;
    ...
    public SXSSFxlsxUtil(){
    	this.wb = new SXSSFWorkbook(10000); // 디스크로 flush되기 전까지 메모리에 들고있는 행의 개수(기본 크기는 100 이며, -1 지정시 무제한이다.) 
    									   // 다른 활용법으로는 생성시 XSSF로 만든 workbook을 넣고 두번쨰 파라미터로 위의 개수를 설정해줘도 된다.
    ...
    /*
     *  이슈
     *  	- autoSizeColumn은 성능이슈가 생긴다 : row수가 많은 경우 매우매우 느려진다.
     *  	- 3000건 받는데 한 10분여 정도 걸린듯
     *  
     *  해결방안
     *  	- 1. 변수(엑셀다운로드해보면서 조절한 정수) * 256으로 셀넓이를 설정한다.
     *  		=> 그래도 느림. 3005건 기준 13초 내외로 걸림..
     *  	- 2. 미리 양식을 만들어놓고 contents영역만 맵핑한다.(colum size 지정 X)
     *  		=> binding util 만들어서 적용
     *  			: 결과적으로 타이틀 같이 커스텀 할꺼면 SXSSF Util을 쓰고 값만 쭉 넣어줄꺼면 Binding Util에서 타이틀 병합 부분 지우고 쓰기. (1번과 2번의 속도 차이는 미미함.)
     *  
     *	참고
     *		- https://ko.coder.work/so/java/242566
    ...
    public void contentsMake() throws Exception{
    	SXSSFRow contentsRow = null;
    	SXSSFCell contentsCell = null;
...
//				}
		...
		FileInputStream is = null;
		 ...
		 try{
			 OutputStream fileOut=new FileOutputStream(filePath);
				...
				if (mimeType == null) {
					mimeType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"; /*"application/x-msdownload";*/ 
				}
	...
	public void setNewSheet(SXSSFSheet newSheet) {
	...
	public CellStyle getContentsStyle() {
