...
<script>
	...
	/* 등록 */
	function goReg() {
		location.href = '<c:url value="/fileboard/selectComBoardForm.do" />';
	}	
	
	/* 상세 */
	function goView(code, seq) {
		document.listFrm.comBoardCode.value = code;
		document.listFrm.comBoardSeq.value = seq;
		document.listFrm.action = "${pageContext.request.contextPath }/fileboard/selectComBoardView.do";
		document.listFrm.submit();
	}
	
	/* 댓글, 대댓글 페이지로 */
	function goViewForUser(code, seq) {
		document.listFrm.comBoardCode.value = code;
		document.listFrm.comBoardSeq.value = seq;
		document.listFrm.action = "${pageContext.request.contextPath }/fileboard/selectComBoardViewForUser.do";
		document.listFrm.submit();
	}
...
<div id="maininner"><!-- maininner (s)-->
	<form name="listFrm" id="listFrm" method="post">
		...
		<input type="hidden" name="comBoardCode">
		<input type="hidden" name="comBoardSeq">
	...
	<%-- 			<caption>테스트 리스트</caption> --%>
				...
				<thead>
					<tr>
						...
						<th scope="col" style="text-align: center;">게시판구분코드</th>
						<th scope="col" style="text-align: center;">게시판일련번호</th>
					...
					<c:choose>
						<c:when test="${not empty resultList }">
							<c:forEach var="result" items="${resultList}" varStatus="varStatus">
								<c:choose>
									<c:when test="${typeCheck eq 'admin' }">
										<tr style="cursor: pointer;" onclick="goView('${result.comBoardCode}','${result.comBoardSeq}'); return false;">
									</c:when>
									<c:otherwise>
										<tr style="cursor: pointer;" onclick="goViewForUser('${result.comBoardCode}','${result.comBoardSeq}'); return false;">
									</c:otherwise>
								</c:choose>
									<!-- No -->
									<td style="text-align: center;"><c:out value="${paginationInfo.totalRecordCount - (comonVO.pageUnit * (paginationInfo.currentPageNo-1 )) - (varStatus.count-1) }"/></td>
									<td style="text-align: center;"><c:out value="${result.comBoardCode}"/></td>
									<td style="text-align: center;"><c:out value="${result.comBoardSeq}"/></td>
									<td style="text-align: left;"><c:out value="${result.boardTitle}"/></td>
									<td style="text-align: center;"><c:out value="${result.regDt}"/></td>
								</tr>
							</c:forEach>
						</c:when>
						<c:otherwise>
	...
	<c:if test="${typeCheck eq 'admin' }">
		<div>
			<button style="text-align: right;" onclick="goReg(); return false;">등록</button>
		</div>
	</c:if>
